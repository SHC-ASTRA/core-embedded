...
Motor *motorList[4] = {&MotorFL, &MotorBL, &MotorFR,
                       &MotorBR}; // Left motors first, right motors second
...
void setup() {
  ...
  timePulseParameters.pulseLenRatio =
      0x55555555; // Set the pulse ratio to 1/3 * 2^32 to produce 33:67
                  // mark:space

  // When the module is _locked_ to GNSS time, make it generate 1kHz
  timePulseParameters.freqPeriodLock =
      1000; // Set the frequency/period to 1000Hz
  timePulseParameters.pulseLenRatioLock =
      0x80000000; // Set the pulse ratio to 1/2 * 2^32 to produce 50:50
                  // mark:space

  timePulseParameters.flags.bits.active =
      1; // Make sure the active flag is set to enable the time pulse. (Set to 0
         // to disable.)
  timePulseParameters.flags.bits.lockedOtherSet =
      1; // Tell the module to use freqPeriod while locking and freqPeriodLock
  ...
  timePulseParameters.flags.bits.polarity =
      1; // Tell the module that we want the rising edge at the top of second.
...
void loop() {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (millis() - motorList[i]->status1.timestamp < 500) {
        vicCAN.send(CMD_REVMOTOR_FEEDBACK, motorList[i]->getID(),
                    ...
                    motorList[i]->status1.busVoltage * 10,
                    motorList[i]->status1.outputCurrent * 10);
      }
      if (millis() - motorList[i]->status1.timestamp < 500 &&
          millis() - motorList[i]->status2.timestamp < 500) {
        vicCAN.send(58, motorList[i]->getID(),
                    motorList[i]->status2.sensorPosition,
  ...
  if (millis() - lastTurn > 100 && turningToStatus.enabled) {
    ...
    if (millis() > turningToStatus.timeoutStamp) {
      ...
      turningToStatus.targetHeading =
          clamp_angle(turningToStatus.targetHeading);
  ...
  if (millis() - lastFeedback >= 2000) {
    ...
    // BNO (IMU) Data
    vicCAN.send(CMD_DATA_IMU_GYRO, bnoData2[0], bnoData2[1], bnoData2[2],
                calibStatus);
    vicCAN.send(CMD_DATA_IMU_ACCEL_HEADING, bnoData2[3], bnoData2[4],
                bnoData2[5], bnoData2[6]);
  ...
  if (Serial.available()) {
    ...
    std::vector<String> args =
        {}; // Initialize empty vector to hold separated arguments
    parseInput(input,
...
String outputBno() {
  ...
  output = "bno," + String(bnoData2[0]) + ',' + String(bnoData2[1]) + ',' +
           String(bnoData2[2]) + ',' + String(bnoData2[3]) + ',' +
           String(bnoData2[4]) + ',' + String(bnoData2[5]) + ',' +
           String(bnoData2[6]);
...
String outputBmp() {
  ...
  output = "bmp," + String(bmpData[0]) + ',' + String(bmpData[1]) + ',' +
           String(bmpData[2]);
